...
function Home() {
    ...
    return (
        <>
            <div className=" container grid sm:grid-cols-1 md:grid-cols-3 mx-auto p-2 py-8 gap-y-8 md:gap-x-24 " >
                <div className={`flex flex-col text-center sm:justify-center md:justify-start col-span-1 `}>
                    <h1 className={`text-4xl font-serif ${styles.text} text-dark-textContent`}>Seja bem vindo ao blog do Lucas{usuario.name ? ", " + usuario.name : ""}</h1>
                    <p className={`${styles.text} text-dark-textContent`}>Este blog é um projeto realizado durante o bootcamp de React/Java da <span className="font-bold">Generation Brazil</span>.
            ...
            <div className={`flex items-center justify-center fixed w-[100%] h-screen bg-dark-background1 ${styles.back}`} 
             ></div>
        </>
    )
}
